...
const QrScanPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchOrderDetails = async () => {
      try {
        ...
        const response = await api.get('/api/qrCode/product', {
          params: { orderId }
        });
  ...
  const sendCheckIn = useCallback(async (uuid) => {
    ...
    try {
      // ✅ [Refactor] fetch -> api.patch (credentials, headers 자동 처리됨)
      await api.patch(`/api/seller/orders/${orderId}/checkin`, { uuid });
