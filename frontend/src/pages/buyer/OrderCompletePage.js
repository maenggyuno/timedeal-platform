...
const OrderCompletePage = () => {
  ...
  // useEffect 내부의 fetch 부분을 아래 async 함수로 교체
  useEffect(() => {
    ...
    // axiosInstance를 사용하는 방식으로 변경
    const getOrderData = async () => {
      try {
        // axiosInstance가 baseURL(api.dongnekok.shop)을 붙여서 요청을 보냅니다.
        ...
        // 서버에서 내려주는 에러 메시지가 있다면 그걸 쓰고, 없으면 기본 메시지 사용
    ...
    return (
      <>
        ...
        <div className={styles.container}>
          <div className={styles.card}><p>오류: {error}</p></div>
        </div>
        <Footer/>
      </>
    );
  ...
  if (orders.length === 0) {
    return (
      <>
        <Header/>
        <div className={styles.container}>
          <div className={styles.card}><p>주문 정보를 불러오는 중...</p></div>
        </div>
        <Footer/>
      </>
    );
