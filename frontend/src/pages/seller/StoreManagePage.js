...
const StoreInfoTab = ({ storeId }) => {
    ...
    const handleSaveHours = () => handleSave('hours', { openingTime: storeData.openingTime, closingTime: storeData.closingTime });
    ...
    return (
        <div className={styles.form}>
            <EditableField label="매장명" onSave={handleSaveName}>
                {(isEditing) => <input type="text" value={storeData.name} onChange={(e) => handleChange('name', e.target.value)} disabled={!isEditing} />}
            </EditableField>
                ...
                {(isEditing) => (
                    <div className={styles.addressGroup}>
                        <div className={styles.postcodeGroup}>
                            ...
                            <button type="button" onClick={handleOpenPostcode} disabled={!isEditing} className={styles.smallButton}>우편번호 찾기</button>
                        </div>
                        <input type="text" value={baseAddress} placeholder="주소" readOnly />
                        <input ref={detailAddressRef} type="text" value={detailAddress} onChange={(e) => setDetailAddress(e.target.value)} placeholder="상세주소" disabled={!isEditing} />
                    </div>
                )}
            </EditableField>

            <EditableField label="전화번호" onSave={handleSavePhoneNumber}>
                {(isEditing) => <input type="tel" value={storeData.phoneNumber} onChange={(e) => handleChange('phoneNumber', e.target.value.replace(/[^0-9-]/g, ''))} disabled={!isEditing} />}
            </EditableField>

            <EditableField label="영업 시간" onSave={handleSaveHours}>
                {(isEditing) => (
                     <div className={styles.timeGroup}>
                        <input type="time" value={storeData.openingTime} onChange={(e) => handleChange('openingTime', e.target.value)} disabled={!isEditing} />
                        <span>~</span>
                        <input type="time" value={storeData.closingTime} onChange={(e) => handleChange('closingTime', e.target.value)} disabled={!isEditing} />
                    </div>
                )}
            </EditableField>
            ...
            <EditableField label="결제 방식" onSave={handleSavePaymentMethod}>
                {(isEditing) => (
                    <select value={storeData.paymentMethod} onChange={(e) => handleChange('paymentMethod', Number(e.target.value))} disabled={!isEditing}>
...
const EmployeeListTab = ({ storeId, refreshEmployees }) => {
    ...
    const handleToggleAll = (e) => {
        ...
        if (e.target.checked) {
            filteredEmployees.forEach(emp => { newCheckedState[emp.userId] = true; });
        }
    ...
    return (
        <div className={styles.employeeSubTabContainer}>
            <div className={styles.listHeader}>
                <input type="text" placeholder="이름 또는 이메일로 검색" className={styles.searchBar} value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
            </div>
                ...
                <table className={`${styles.employeeTable} ${styles.employeeListTable}`}>
                    ...
                    <thead>
                        <tr>
                            ...
                            <th>이름</th><th>이메일</th><th>직위</th>
                        </tr>
                    </thead>
                    <tbody>
                        {filteredEmployees.map(emp => (
                            <tr key={emp.userId}>
                                <td><input type="checkbox" checked={!!checkedState[emp.userId]} onChange={() => handleToggleCheck(emp.userId)} /></td>
                                <td>{emp.name}</td><td>{emp.email}</td><td>{emp.position}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
                ...
                <table className={`${styles.employeeTable} ${styles.addEmployeeTable}`}>
                    ...
                    <thead>
                        <tr>
                            ...
                            <th>이름</th><th>이메일</th><th>직위</th><th></th>
                        </tr>
                    </thead>
                    <tbody>
                        {stagedEmployees.map(emp => (
                            <tr key={emp.userId}>
                                ...
                                <td>{emp.name}</td><td>{emp.email}</td><td>직원</td>
                                <td><button className={styles.removeButton} onClick={() => handleRemoveStagedEmployee(emp.userId)}>-</button></td>
                            </tr>
                        ))}
                    </tbody>
                </table>
            ...
            {isModalOpen && (
                <div className={styles.modalBackdrop} onClick={closeModal}>
                    <div className={styles.modalContent} onClick={e => e.stopPropagation()}>
                        <h4>직원 이메일로 추가</h4>
                        <div className={styles.inputGroup}>
                            <input type="email" placeholder="직원의 이메일을 입력하세요" value={emailInput} onChange={e => setEmailInput(e.target.value)} disabled={isSearching} />
                            <button className={styles.smallButton} onClick={handleCheckEmail} disabled={isSearching}>
                                {isSearching ? '확인 중...' : '확인하기'}
                            </button>
                        </div>
                        {searchResult?.status === 'found' && <p className={styles.successText}>{searchResult.user.name}({searchResult.user.email}) 님을 추가할 수 있습니다.</p>}
                        {searchResult && searchResult.status !== 'found' && <p className={styles.errorText}>{searchResult.message}</p>}
                        <div className={styles.modalActions}>
                            <button className={styles.cancelButton} onClick={closeModal}>취소</button>
                            <button className={styles.addButton} onClick={handleAddStagedEmployee} disabled={searchResult?.status !== 'found'}>추가하기</button>
                        </div>
                    </div>
                </div>
            )}
    ...
    return (
        <div>
            <div className={styles.employeeNavTabs}>
                <button className={`${styles.employeeNavTab} ${employeeActiveTab === 'list' ? styles.active : ''}`} onClick={() => setEmployeeActiveTab('list')}>직원 목록</button>
                <button className={`${styles.employeeNavTab} ${employeeActiveTab === 'add' ? styles.active : ''}`} onClick={() => setEmployeeActiveTab('add')}>직원 추가</button>
            </div>
        ...
        <>
            ...
            <div className={styles.container}>
                ...
                <div className={styles.navTabs}>
                    <button className={`${styles.navTab} ${activeTab === 'store' ? styles.active : ''}`} onClick={() => setActiveTab('store')}>매장 정보</button>
                    <button className={`${styles.navTab} ${activeTab === 'employee' ? styles.active : ''}`} onClick={() => setActiveTab('employee')}>직원 관리</button>
                </div>
