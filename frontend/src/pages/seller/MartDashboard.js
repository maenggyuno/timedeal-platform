...
const MartDashboard = () => {
  ...
  const fetchData = useCallback(async (endpoint, setter, page) => {
    try {
      const response = await api.get(`/api/seller/stores/${martId}/dashboard/${endpoint}`, {
        params: {page} // 쿼리 파라미터도 이렇게 넣으면 깔끔함
      });
  ...
  useEffect(() => {
    fetchData('reservations', setReservations, resPage);
  }, [martId, resPage, fetchData]);
  useEffect(() => {
    fetchData('purchases', setPurchases, purPage);
  }, [martId, purPage, fetchData]);
  useEffect(() => {
    fetchData('completed', setCompleted, comPage);
  }, [martId, comPage, fetchData]);
